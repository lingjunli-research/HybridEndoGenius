...
help_message() {
    ...
    echo
    echo
    echo "Syntax: bash make_dag.sh -s <sample_basename> -j <round1_json> -k <round2_json> -o <output_filename>"
    ...
    echo
    echo "Example usage: bash make_dag.sh -s sample -j round1.json -k round2.json -o ms_pipeline"
    exit 1
}
...
# Parse flags
while getopts "s:j:k:o:h" flag; do
    case "${flag}" in
...
if [[ -z "$SAMPLE_BASE" || -z "$ROUND1_JSON" || -z "$ROUND2_JSON" || -z "$FILENAME" ]]; then
...
echo "VARS ANALYZE_FDR target_mztab=\"$TARGET_MZTAB\" decoy_mztab=\"$DECOY_MZTAB\" motif_fasta=\"$MOTIF_FASTA\" filter_sif=\"$FILTER_SIF\"" >> "$FILENAME.dag"
